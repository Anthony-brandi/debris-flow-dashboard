...
def load_fire_perimeters():
    try:
        actual_zip = 'Master_Fire_Dataset.zip'
        if not os.path.exists(actual_zip): return None
...
# ==========================================
    ...
    if run_analysis:
        with st.spinner("Requesting Satellite Arrays..."):
            try:
                ...
                <i style="background:#8c510a; width:12px; height:12px; float:left; margin-right:5px;"></i> High Soil Erodibility
                </div>"""

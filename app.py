...
if not cal_fires.empty:
    ...
    # Store in session state so validation tab can access without visiting Module 3
...
# ==========================================
    ...
    with st.spinner("Compiling Spatial Intersection Data..."):
        ...
        if basemap_choice == "Satellite":
            m2 = folium.Map(location=[centroid.y, centroid.x], zoom_start=12,
        ...
        folium.GeoJson(fire_data.geometry,
            style_function=lambda x: {'fillColor': 'transparent', 'color': perimeter_color,
                                      'weight': 2.5, 'dashArray': '5, 5'}).add_to(m2)

        C_RISK='#FF5733'; C_SLOPE='yellow'; C_CONCAVITY='#8e44ad'
        C_SEVERITY='red';  C_STREAMS='#3498db'; C_ROADS='#2ecc71'

        if show_slope:
            folium.TileLayer(tiles=slope_mask.selfMask().getMapId({'palette':[C_SLOPE]})['tile_fetcher'].url_format,
                attr='USGS',name='Slope',opacity=0.4).add_to(m2)
        if show_concavity:
            folium.TileLayer(tiles=concavity_mask.selfMask().getMapId({'palette':[C_CONCAVITY]})['tile_fetcher'].url_format,
                attr='USGS',name='Concavity',opacity=0.6).add_to(m2)
        if show_severity:
            folium.TileLayer(tiles=severity_mask.selfMask().getMapId({'palette':[C_SEVERITY]})['tile_fetcher'].url_format,
                attr='ESA',name='Severity',opacity=0.4).add_to(m2)
        if show_soils:
            folium.TileLayer(tiles=erodible_soils.getMapId(
                {'min':10,'max':80,'palette':['#f4a460','#d2691e','#8b4513']})['tile_fetcher'].url_format,
                attr='Soil',name='Soils',opacity=0.7).add_to(m2)
        if show_streams:
            folium.TileLayer(tiles=streams_img.getMapId({'palette':[C_STREAMS]})['tile_fetcher'].url_format,
                attr='WWF',name='Streams').add_to(m2)
        if show_roads:
            folium.TileLayer(tiles=roads_img.getMapId({'palette':[C_ROADS]})['tile_fetcher'].url_format,
                attr='TIGER',name='Roads').add_to(m2)
        if show_risk:
            folium.TileLayer(tiles=hazard_intersection.getMapId(
                {'min':1,'max':1,'palette':[C_RISK]})['tile_fetcher'].url_format,
                attr='GEE',name='Risk',opacity=0.8).add_to(m2)

        toggle_key = (f"lab_{selected_fire}_v{show_risk}{show_slope}{show_concavity}"
...
# ==========================================
    ...
    with st.spinner(f"Running Gartner (2014) regression at {design_storm_mmhr} mm/hr..."):
        ...
        huc12_processed = combined_img.reduceRegions(
            collection=huc12, reducer=ee.Reducer.sum(), scale=250, tileScale=16
            ...
            if not gdf.empty:
                ...
                folium.GeoJson(fire_data.geometry,
                    style_function=lambda x: {'fillColor': 'transparent', 'color': 'black',
                                              'weight': 2, 'dashArray': '5, 5'}).add_to(m3)
                folium.Choropleth(
                    geo_data=gdf, name='Sediment Yield', data=df_results,
                ...
                stream_vis = stream_img.getMapId({'min':0,'max':2.5,'palette':['#00b4d8','#0077b6','#03045e']})
                folium.TileLayer(tiles=stream_vis['tile_fetcher'].url_format,
                    attr='WWF', name='Stream Transport (Discharge)', overlay=True).add_to(m3)
                folium.GeoJson(gdf,
...
# ==========================================
    ...
    with tab1:
        ...
        which geomorphic factors make the landscape vulnerable.
    ...
    with tab2:
        ...
        **Validation:** Thomas Fire hindcast predicted Matilija Creek at 26,511 m³,
        consistent with USGS field documentation (Lancaster et al., 2021).

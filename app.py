...
if page == "Interactive Risk Map":
    try:
        ...
        with st.sidebar.expander("Map Layer Settings", expanded=True):
            ...
            show_precip = st.checkbox("Precipitation (NASA GPM)", value=True) # Enabled for testing
            show_risk = st.checkbox("Hazard Intersection", value=False)
            show_watersheds = st.checkbox("Watershed Outlines", value=False)
            show_infra = st.checkbox("Infrastructure (Roads)", value=False)
        ...
        if analyze_btn:
            with st.spinner("Processing multispectral and rainfall data..."):
                ...
                # --- PRECIPITATION (NASA GPM V07 - FIXED TRANSPARENCY) ---
                ...
                <i style="background:#084594; width:10px; height:10px; float:left; margin-right:5px;"></i> Heavy Rainfall<br>
                ...
                if show_precip:
                    # FIXED: Masking precip to remove "white-out". Only shows rain > 1mm.
                    p_vis = {'min': 1, 'max': 150, 'palette': ['#f7fbff','#deebf7','#9ecae1','#4292c6','#084594']}
                    precip_masked = precip.updateMask(precip.gt(1))
                    folium.TileLayer(tiles=precip_masked.getMapId(p_vis)['tile_fetcher'].url_format, attr='NASA', name='Rainfall', opacity=0.5).add_to(m)

                if show_recovery:
                    vis = {'min': 0.1, 'max': 0.5, 'palette': ['#ffffb2', '#fecc5c', '#fd8d3c', '#f03b20', '#bd0026']}
                    dnbr_masked = dnbr.updateMask(dnbr.gt(0.1))
                    folium.TileLayer(tiles=dnbr_masked.getMapId(vis)['tile_fetcher'].url_format, attr='S2', name='Burn Status', opacity=0.6).add_to(m)
...
# ==========================================
    ...
    st.write("1. Select an incident.\n2. Adjust your slope and time sliders.\n3. Run analysis to identify where rain impacts burned watersheds.")

elif page == "Technical Documentation":
    st.title("Technical Methodology")
    st.write("Detailed methodology on dNBR spectral indices, NASA GPM rainfall integration, and topographic thresholds.")

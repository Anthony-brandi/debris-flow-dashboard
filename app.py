...
if page == "1. Incident Briefing" and all_fires is not None:
    ...
    with col2:
        ...
        st.info("Debris flow risk is assessed by intersecting burn severity, slope, and soil erodibility.")
        if selected_name == "DIXIE":
            st.warning("Case Study: Extreme topographic relief and infrastructure intersection.")
...
# ==========================================
    ...
    if run_analysis:
        with st.spinner("Processing multispectral and soil datasets..."):
            try:
                ...
                # Math Engine - Widened search window to ensure cloud-free imagery
...
# ==========================================
    ...
    if run_stats:
        with st.spinner("Reducing basin statistics..."):
            try:
                area = ee.FeatureCollection(fire_subset.__geo_interface__)
                pre_date = ee.Date(manual_baseline.strftime('%Y-%m-%d'))
                fire_start = ee.Date(default_alarm_dt.strftime('%Y-%m-%d'))
                ...
                else: st.warning("Adjust thresholds to view results for this perimeter.")
            except Exception as e: st.error(f"Stat Error: {e}")
